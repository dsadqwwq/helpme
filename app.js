...
function showPanel(which, data, tokenId){
  ...
  if(which==="claim") els.claimBtn.onclick=()=>{ const ids=room(targets); if(!ids.length) return toast("No tiles"); ids.forEach(id=>pushBatch({type:"claim", tokenId:id})); };
  if(which==="list") els.listBtn.onclick=()=>{ const v=(els.listPrice.value||"0").trim(); if(!/^[0-9]+(\.[0-9]+)?$/.test(v)) return toast("Bad price"); const wei=toWei(v); const ids=room(targets); if(!ids.length) return toast("No tiles"); ids.forEach(id=>pushBatch({type:"list", tokenId:id, priceWei:wei})); };
  if(which==="buy"){ els.buyBtn.onclick=()=>{ const ids=room(targets); if(!ids.length) return toast("No tiles"); ids.forEach(id=>pushBatch({type:"buy", tokenId:id, priceWei:data.priceWei})); }; els.cancelBtn.onclick=()=>{ const ids=room(targets); if(!ids.length) return toast("No tiles"); ids.forEach(id=>pushBatch({type:"cancel", tokenId:id})); };
}
function pushBatch(it){ if(batch.length>=10){ toast("Batch full"); return;} batch.push(it); const li=document.createElement("li"); li.textContent=`${it.type.toUpperCase()} #${it.tokenId}`+(it.priceWei?` @ ${Number(it.priceWei)/1e18} ETH`:""); els.batchList.appendChild(li); els.batchCount.textContent=`(${batch.length}/10)`; }
...
function init(){
  ...
  els.clearSelection.addEventListener("click", ()=>{ selected.clear(); setupCanvas(); els.selCount.textContent=""; });
